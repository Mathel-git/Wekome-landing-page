import {
  BatteryCharging,
  Cable,
  Plane,
  ShieldCheck,
  ShoppingBag,
  Smartphone,
  Zap,
} from "lucide-react";

import productData from "../data/productData";

const iconMap = {
  zap: Zap,
  cable: Cable,
  smartphone: Smartphone,
  battery: BatteryCharging,
  shield: ShieldCheck,
  plane: Plane,
};

function WhatsAppIcon({ size = 22 }) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="currentColor"
      xmlns="http://www.w3.org/2000/svg"
      aria-hidden="true"
    >
      <path d="M20.52 3.48A11.82 11.82 0 0 0 12.05 0C5.5 0 .18 5.32.18 11.87c0 2.09.55 4.13 1.59 5.93L.08 24l6.35-1.66a11.86 11.86 0 0 0 5.62 1.43h.01c6.54 0 11.86-5.32 11.86-11.87 0-3.17-1.23-6.15-3.4-8.42ZM12.06 21.77h-.01a9.86 9.86 0 0 1-5.03-1.38l-.36-.21-3.77.99 1.01-3.67-.23-.38a9.87 9.87 0 1 1 8.39 4.65Zm5.41-7.39c-.3-.15-1.78-.88-2.06-.98-.28-.1-.48-.15-.68.15-.2.3-.78.98-.96 1.18-.18.2-.35.22-.65.07-.3-.15-1.27-.47-2.42-1.5-.89-.79-1.49-1.77-1.67-2.07-.18-.3-.02-.46.14-.61.14-.14.3-.35.45-.52.15-.18.2-.3.3-.5.1-.2.05-.37-.02-.52-.07-.15-.68-1.64-.93-2.24-.24-.58-.49-.5-.68-.51h-.58c-.2 0-.52.07-.8.37-.28.3-1.04 1.02-1.04 2.49s1.07 2.89 1.22 3.09c.15.2 2.1 3.21 5.09 4.5.71.31 1.27.5 1.7.64.71.23 1.36.2 1.87.12.57-.09 1.78-.73 2.03-1.43.25-.7.25-1.3.18-1.43-.07-.13-.27-.2-.57-.35Z" />
    </svg>
  );
}

function Hero() {
  return (
    <section
      id="home"
      className="
        relative
        overflow-hidden
        bg-[#05040b]
        pt-[76px]
        text-white
        min-h-[760px]
        sm:min-h-[820px]
        lg:min-h-screen
      "
    >
      {/* =====================================================
          BACKGROUND
      ====================================================== */}

      <div className="pointer-events-none absolute inset-0">
        {/* Main glow */}
        <div
          className="
            absolute
            -right-[220px]
            top-[20px]
            h-[520px]
            w-[520px]
            rounded-full
            bg-purple-700/20
            blur-[120px]
            sm:-right-[180px]
            sm:h-[650px]
            sm:w-[650px]
          "
        />

        {/* Lower glow */}
        <div
          className="
            absolute
            right-[0]
            bottom-[0]
            h-[350px]
            w-[350px]
            rounded-full
            bg-purple-600/10
            blur-[110px]
            sm:h-[450px]
            sm:w-[450px]
          "
        />

        {/* Decorative rings */}
        <div
          className="
            absolute
            -right-[300px]
            -top-[280px]
            h-[650px]
            w-[650px]
            rounded-full
            border
            border-purple-500/20
            sm:-right-[250px]
            sm:-top-[220px]
            sm:h-[750px]
            sm:w-[750px]
          "
        />

        <div
          className="
            absolute
            -right-[280px]
            -top-[250px]
            h-[600px]
            w-[600px]
            rounded-full
            border
            border-purple-400/20
            sm:-right-[230px]
            sm:-top-[200px]
            sm:h-[700px]
            sm:w-[700px]
          "
        />
      </div>

      {/* =====================================================
          HERO CONTAINER
      ====================================================== */}

      <div
        className="
          relative
          z-10
          mx-auto
          max-w-[1440px]
          px-4
          sm:px-7
          lg:min-h-[calc(100vh-76px)]
          lg:px-12
        "
      >
        {/* ===================================================
            MOBILE / TABLET COMPOSITION
            ---------------------------------------------------
            Text remains LEFT.
            Product remains RIGHT.
            Product starts near WP-66.
        ==================================================== */}

        <div
          className="
            relative
            min-h-[684px]
            lg:hidden
          "
        >
          {/* ================================================
              LEFT CONTENT
          ================================================= */}

          <div
            className="
              relative
              z-30
              w-[58%]
              pt-8
              sm:w-[56%]
              sm:pt-10
            "
          >
            {/* Product badge */}

            <div className="mb-5 flex items-center gap-2 sm:mb-6 sm:gap-3">
              <span
                className="
                  rounded-md
                  bg-purple-600
                  px-2.5
                  py-1.5
                  text-[8px]
                  font-bold
                  tracking-wider
                  sm:px-3
                  sm:text-[10px]
                "
              >
                {productData.model}
              </span>

              <span
                className="
                  whitespace-nowrap
                  text-[8px]
                  font-medium
                  tracking-[0.12em]
                  text-gray-400
                  sm:text-xs
                "
              >
                {productData.series}
              </span>
            </div>

            {/* Heading */}

            <h1
              className="
                max-w-[350px]
                text-[38px]
                font-black
                leading-[0.91]
                tracking-[-0.055em]
                sm:text-[52px]
                md:text-[62px]
              "
            >
              Power That
              <br />

              <span
                className="
                  bg-gradient-to-r
                  from-purple-400
                  via-fuchsia-500
                  to-purple-500
                  bg-clip-text
                  text-transparent
                "
              >
                Goes With You.
              </span>
            </h1>

            {/* Description */}

            <p
              className="
                mt-5
                max-w-[330px]
                text-[11px]
                leading-5
                text-gray-400
                sm:mt-7
                sm:text-base
                sm:leading-7
              "
            >
              {productData.description}
            </p>

            {/* Features */}

            <div
              className="
                mt-7
                grid
                grid-cols-2
                gap-x-3
                gap-y-5
                sm:mt-9
                sm:grid-cols-3
                sm:gap-5
              "
            >
              {productData.features.slice(0, 5).map((feature) => {
                const Icon = iconMap[feature.icon];

                return (
                  <div
                    key={feature.title}
                    className="min-w-0"
                  >
                    <div
                      className="
                        mb-2
                        flex
                        h-8
                        w-8
                        items-center
                        justify-center
                        rounded-full
                        border
                        border-purple-500/30
                        bg-purple-500/10
                        text-purple-300
                        sm:h-10
                        sm:w-10
                      "
                    >
                      <Icon size={14} />
                    </div>

                    <p
                      className="
                        text-[7px]
                        font-bold
                        tracking-wide
                        text-white
                        sm:text-[9px]
                      "
                    >
                      {feature.title}
                    </p>

                    <p
                      className="
                        mt-1
                        text-[6px]
                        leading-3
                        text-gray-500
                        sm:text-[8px]
                        sm:leading-4
                      "
                    >
                      {feature.description}
                    </p>
                  </div>
                );
              })}
            </div>

            {/* Price */}

            <div
              className="
                mt-7
                grid
                w-[calc(100vw-32px)]
                max-w-[660px]
                grid-cols-3
                overflow-hidden
                rounded-xl
                border
                border-white/10
                bg-white/[0.03]
              "
            >
              <div className="px-3 py-3">
                <p className="text-[6px] uppercase tracking-wider text-gray-500">
                  Original Price
                </p>

                <p className="mt-1 text-[13px] font-bold text-gray-400 line-through">
                  {productData.originalPrice}
                </p>
              </div>

              <div className="bg-purple-600/20 px-3 py-3">
                <p className="text-[6px] uppercase tracking-wider text-purple-300">
                  Promo Price
                </p>

                <p className="mt-1 text-[17px] font-black text-yellow-300">
                  {productData.promoPrice}
                </p>
              </div>

              <div className="px-3 py-3">
                <p className="text-[6px] uppercase tracking-wider text-gray-500">
                  You Save
                </p>

                <p className="mt-1 text-[13px] font-bold text-white">
                  {productData.savings}
                </p>
              </div>
            </div>

            {/* Buttons */}

            <div
              className="
                mt-5
                flex
                w-[calc(100vw-32px)]
                max-w-[660px]
                gap-2
              "
            >
              {/* WhatsApp */}

              <a
                href={`https://wa.me/${productData.whatsapp}`}
                target="_blank"
                rel="noopener noreferrer"
                className="
                  flex
                  min-w-0
                  flex-1
                  items-center
                  justify-center
                  gap-2
                  rounded-lg
                  bg-gradient-to-r
                  from-purple-600
                  to-fuchsia-500
                  px-2
                  py-3
                  shadow-[0_0_30px_rgba(168,85,247,0.25)]
                "
              >
                <span
                  className="
                    flex
                    h-7
                    w-7
                    shrink-0
                    items-center
                    justify-center
                    rounded-full
                    bg-white/10
                  "
                >
                  <WhatsAppIcon size={17} />
                </span>

                <span className="min-w-0 text-left">
                  <span className="block text-[6px] font-medium uppercase tracking-wider opacity-70">
                    Order Now on WhatsApp
                  </span>

                  <span className="block truncate text-[10px] font-bold">
                    08162409010
                  </span>
                </span>
              </a>

              {/* Shop */}

              <a
                href="#order"
                className="
                  flex
                  w-[115px]
                  shrink-0
                  items-center
                  justify-center
                  gap-2
                  rounded-lg
                  border
                  border-purple-500/30
                  bg-white/[0.03]
                  px-3
                  py-3
                  text-[9px]
                  font-bold
                "
              >
                <ShoppingBag size={15} />
                SHOP NOW
              </a>
            </div>
          </div>

          {/* ================================================
              MOBILE PRODUCT
              Starts near WP-66 line.
          ================================================= */}

          <div
            className="
              pointer-events-none
              absolute
              right-[-12px]
              top-[72px]
              z-20
              h-[430px]
              w-[47%]
              sm:right-[-5px]
              sm:top-[76px]
              sm:h-[500px]
              sm:w-[45%]
            "
          >
            {/* Product glow */}

            <div
              className="
                absolute
                left-1/2
                top-[38%]
                h-[260px]
                w-[180px]
                -translate-x-1/2
                -translate-y-1/2
                rounded-full
                bg-purple-600/30
                blur-[80px]
                sm:h-[340px]
                sm:w-[240px]
                sm:blur-[100px]
              "
            />

            {/* Power bank */}

            <div
              className="
                absolute
                left-1/2
                top-[5px]
                flex
                -translate-x-1/2
                items-start
                justify-center
              "
            >
              <img
                src="/images/products/powerbank-hero.png"
                alt="WEKOME WP-66 20,000mAh Power Bank"
                className="
                  h-auto
                  w-[250px]
                  max-w-none
                  object-contain
                  drop-shadow-[0_30px_70px_rgba(0,0,0,0.75)]
                  sm:w-[340px]
                "
              />
            </div>

            {/* Capacity */}

            <div
              className="
                absolute
                right-[-3px]
                top-[120px]
                w-[112px]
                rounded-xl
                border
                border-purple-400/30
                bg-black/70
                p-3
                backdrop-blur-xl
                sm:top-[145px]
                sm:w-[150px]
                sm:p-4
              "
            >
              <div className="flex items-end gap-1">
                <span className="text-xl font-black sm:text-2xl">
                  {productData.capacity}
                </span>

                <span className="mb-1 text-[8px] font-bold text-purple-400 sm:text-xs">
                  {productData.capacityUnit}
                </span>
              </div>

              <p className="mt-1 text-[6px] uppercase tracking-[0.2em] text-gray-500 sm:text-[8px]">
                Real Capacity
              </p>

              <div className="mt-2 h-px bg-white/10" />

              <p className="mt-2 text-[8px] text-gray-300 sm:text-[10px]">
                More power.
                <br />
                Less worry.
              </p>
            </div>

            {/* Benefits */}

            <div
              className="
                absolute
                right-[-4px]
                top-[280px]
                w-[145px]
                space-y-3
                sm:top-[335px]
                sm:w-[175px]
                sm:space-y-4
              "
            >
              {productData.benefits.slice(0, 4).map((benefit) => {
                const Icon = iconMap[benefit.icon];

                return (
                  <div
                    key={benefit.title}
                    className="flex items-center gap-2"
                  >
                    <div
                      className="
                        flex
                        h-7
                        w-7
                        shrink-0
                        items-center
                        justify-center
                        rounded-full
                        border
                        border-purple-500/30
                        bg-purple-500/10
                        text-purple-300
                        sm:h-9
                        sm:w-9
                      "
                    >
                      <Icon size={13} />
                    </div>

                    <div className="min-w-0">
                      <p className="truncate text-[7px] font-bold text-white sm:text-[9px]">
                        {benefit.title}
                      </p>

                      <p className="mt-0.5 truncate text-[6px] text-gray-500 sm:text-[8px]">
                        {benefit.description}
                      </p>
                    </div>
                  </div>
                );
              })}
            </div>
          </div>
        </div>

        {/* ===================================================
            DESKTOP
        ==================================================== */}

        <div
          className="
            hidden
            min-h-[calc(100vh-76px)]
            items-center
            lg:grid
            lg:grid-cols-[1fr_1fr]
            lg:gap-0
          "
        >
          {/* LEFT */}

          <div className="relative z-20 max-w-[650px] pt-6">
            {/* Badge */}

            <div className="mb-6 flex items-center gap-3">
              <span className="rounded-md bg-purple-600 px-3 py-1.5 text-[10px] font-bold tracking-wider">
                {productData.model}
              </span>

              <span className="text-xs font-medium tracking-[0.15em] text-gray-400">
                {productData.series}
              </span>
            </div>

            {/* Heading */}

            <h1 className="max-w-[700px] text-[84px] font-black leading-[0.94] tracking-[-0.05em]">
              Power That
              <br />

              <span className="bg-gradient-to-r from-purple-400 via-fuchsia-500 to-purple-500 bg-clip-text text-transparent">
                Goes With You.
              </span>
            </h1>

            {/* Description */}

            <p className="mt-7 max-w-[500px] text-lg leading-7 text-gray-400">
              {productData.description}
            </p>

            {/* Features */}

            <div className="mt-9 grid max-w-[650px] grid-cols-5 gap-5">
              {productData.features.slice(0, 5).map((feature) => {
                const Icon = iconMap[feature.icon];

                return (
                  <div key={feature.title}>
                    <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-full border border-purple-500/30 bg-purple-500/10 text-purple-300">
                      <Icon size={17} />
                    </div>

                    <p className="text-[9px] font-bold text-white">
                      {feature.title}
                    </p>

                    <p className="mt-1 text-[9px] leading-4 text-gray-500">
                      {feature.description}
                    </p>
                  </div>
                );
              })}
            </div>

            {/* Price */}

            <div className="mt-9 grid max-w-[665px] grid-cols-3 overflow-hidden rounded-xl border border-white/10 bg-white/[0.03]">
              <div className="px-5 py-4">
                <p className="text-[9px] uppercase tracking-wider text-gray-500">
                  Original Price
                </p>

                <p className="mt-1 text-lg font-bold text-gray-400 line-through">
                  {productData.originalPrice}
                </p>
              </div>

              <div className="bg-purple-600/20 px-5 py-4">
                <p className="text-[9px] uppercase tracking-wider text-purple-300">
                  Promo Price
                </p>

                <p className="mt-1 text-2xl font-black text-yellow-300">
                  {productData.promoPrice}
                </p>
              </div>

              <div className="px-5 py-4">
                <p className="text-[9px] uppercase tracking-wider text-gray-500">
                  You Save
                </p>

                <p className="mt-1 text-lg font-bold text-white">
                  {productData.savings}
                </p>
              </div>
            </div>

            {/* Buttons */}

            <div className="mt-7 flex gap-3">
              <a
                href={`https://wa.me/${productData.whatsapp}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-3 rounded-lg bg-gradient-to-r from-purple-600 to-fuchsia-500 px-7 py-4 text-sm font-bold shadow-[0_0_30px_rgba(168,85,247,0.25)]"
              >
                <span className="flex h-8 w-8 items-center justify-center rounded-full bg-white/10">
                  <WhatsAppIcon size={18} />
                </span>

                <span className="text-left">
                  <span className="block text-[8px] font-medium uppercase tracking-wider opacity-70">
                    Order Now on WhatsApp
                  </span>

                  <span className="text-sm">
                    08162409010
                  </span>
                </span>
              </a>

              <a
                href="#order"
                className="flex items-center justify-center gap-3 rounded-lg border border-purple-500/30 bg-white/[0.03] px-8 py-4 text-sm font-bold"
              >
                <ShoppingBag size={17} />
                SHOP NOW
              </a>
            </div>
          </div>

          {/* RIGHT PRODUCT */}

          <div className="relative flex h-[650px] items-center justify-center">
            {/* Glow */}

            <div className="absolute h-[430px] w-[330px] rounded-full bg-purple-600/30 blur-[110px]" />

            {/* Product */}

            <img
              src="/images/products/powerbank-hero.png"
              alt="WEKOME WP-66 20,000mAh Power Bank"
              className="
                relative
                z-10
                w-full
                max-w-[520px]
                object-contain
                drop-shadow-[0_30px_70px_rgba(0,0,0,0.7)]
              "
            />

            {/* Capacity */}

            <div className="absolute right-0 top-[12%] z-20 w-[190px] rounded-2xl border border-purple-400/30 bg-black/70 p-5 backdrop-blur-xl">
              <div className="flex items-end gap-1">
                <span className="text-3xl font-black">
                  {productData.capacity}
                </span>

                <span className="mb-1 text-sm font-bold text-purple-400">
                  {productData.capacityUnit}
                </span>
              </div>

              <p className="mt-2 text-[9px] uppercase tracking-[0.2em] text-gray-500">
                Real Capacity
              </p>

              <div className="mt-3 h-px bg-white/10" />

              <p className="mt-3 text-xs text-gray-300">
                More power.
                <br />
                Less worry.
              </p>
            </div>

            {/* Benefits */}

            <div className="absolute bottom-4 right-0 z-20 w-[220px] space-y-4">
              {productData.benefits.slice(0, 4).map((benefit) => {
                const Icon = iconMap[benefit.icon];

                return (
                  <div
                    key={benefit.title}
                    className="flex items-center gap-3"
                  >
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-purple-500/30 bg-purple-500/10 text-purple-300">
                      <Icon size={16} />
                    </div>

                    <div>
                      <p className="text-[9px] font-bold text-white">
                        {benefit.title}
                      </p>

                      <p className="mt-1 text-[8px] text-gray-500">
                        {benefit.description}
                      </p>
                    </div>
                  </div>
                );
              })}
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}

export default Hero;